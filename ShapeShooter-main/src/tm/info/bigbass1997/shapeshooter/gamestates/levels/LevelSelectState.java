...
public class LevelSelectState extends GameState {
	...
	public void update(float delta) {
		for(int i = 0; i < buttons.size(); i++){
			...
			if(b.getName().length() == 1 && b.isPressed() && SettingsManager.unlocked.get(Integer.valueOf(b.getName()) - 1)) gsm.setState(Integer.valueOf(b.getName()) + 100);
		}
	}
	...
	public void draw() {
		...
		for(int i = 0; i < SettingsManager.unlocked.size() - 1; i++){
			if(!SettingsManager.unlocked.get(i)){
		...
		int l = SettingsManager.unlocked.size();
		if(!SettingsManager.unlocked.get(l - 1)){
